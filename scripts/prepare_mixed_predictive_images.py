...
def prepare_data(imgs_path: str, categories: List[str], iterations: List[str],
    ...
    subfolders = product(models, categories, iterations, map(lambda x: f'top_{x}', tops), 
                         [*list(WEIGHTING_SCHEMES.values()), 'unweighted'])
    for model, cat, iteration, top, weighting_scheme in subfolders:
        os.makedirs(os.path.join(imgs_path, model, cat, iteration, top, weighting_scheme), exist_ok=True)
    ...
    for model in models:
        ...
        for file in files:
            ...
            weighted = ''
            if len(fn_parts) == 5:
                cls, cat, _, top, iteration = fn_parts
            elif len(fn_parts) == 6:
                cls, cat, _, top, weighted, iteration = fn_parts
            elif len(fn_parts) == 7:
                cls, cat, _, _, top, batch_ind, iteration = fn_parts
            elif len(fn_parts) == 8:
                cls, cat, _, _, top, weighted, batch_ind, iteration = fn_parts
            else:
            ...
            new_img_path = os.path.join(imgs_path, model, cat, iteration, top,
...
if __name__ == '__main__':
    ...
    prepare_data(args.images_path, args.categories, args.iterations, args.models, args.tops)